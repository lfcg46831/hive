...
| `HIVE__GATEWAY__NUMBEROFSHARDS` | unset (extractor default `12`) | Number of Cluster Sharding shards for the AI gateway entity type, whose `entityId` is the effective provider id (`Hive:Gateway:NumberOfShards`, US-F1-05-T07). Keep it identical on every node; the entities are not persistent, so changing it only reshuffles live provider buckets. Must be greater than zero when set. |
| `HIVE__GATEWAY__CLUSTERUPTIMEOUT` | unset (workload default `00:00:30`) | Maximum time the `gateway` workloads wait for the `ActorSystem` to reach cluster *Up* before initializing the AI gateway shard region, or its proxy on an `agents` node (`Hive:Gateway:ClusterUpTimeout`, US-F1-05-T07), as a `hh:mm:ss` span. If *Up* is not reached within this window the workload fails the node startup observably (`ClusterStartupTimeoutException`). Must be greater than zero when set. |
| `HIVE__GATEWAY__ASKTIMEOUT` | unset (invoker default `00:05:00`) | Transport timeout for a single gateway call routed to the provider entity (`Hive:Gateway:AskTimeout`, US-F1-05-T07), as a `hh:mm:ss` span. It must exceed the worst case of the resilience pipeline — queue wait plus every retry, its backoff and the provider timeout — otherwise a slow but healthy call is turned into a sanitized `provider-unavailable` transport failure. Must be greater than zero when set. |
