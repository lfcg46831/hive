...
Preparation is local and makes no network calls. It rejects unknown manifest fields, invalid ids/limits/policy, paths outside the repository, SHA-256 drift, duplicate reproducibility roles/paths, missing positions, an invalid source→destination hierarchy, or a provider/model/timeout/output/iteration value that differs from the selected AI position in the organization snapshot. It writes only ignored files under `artifacts/evaluation/experiments/<experiment-id>/`: `compose.env`, which contains interpolation values but no credentials, and `effective-configuration.v1.json`, which contains the normalized portable configuration and its SHA-256. Pass the ignored `.env` first so database/provider secrets remain in their existing owner; never add secrets to the manifest or generated experiment env file.

The generic adapter activates only the scoped `LAB` audit/export profile, selects the declared real-provider default, applies the declared outcome mode and verifier timeout, and bind-mounts the hash-verified organization snapshot read-only. Position model, provider timeout, output ceiling, and iteration limit remain owned by that snapshot and are checked by the tooling. Inspect `docker compose ... config` before creating the container; it is the effective deployment configuration, so a stale generated env file must not be reused after a manifest change.
