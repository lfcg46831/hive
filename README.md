...
F0.8 makes the solution ready to request a new authorized preflight or calibration; it does not satisfy the product-quality gate. The [post-F0.8 local qualification](evidence/preflight/us-f0-17-local-qualification.v1.md#verdict) remains blocked for its reviewed commit; the warning correction is implemented, but the qualification must be repeated on a new reviewed commit before real-provider smokes can be requested. Calibration remains unexecuted, the latest measured candidate was [rejected for freeze](evidence/evaluation/bug-triage-hybrid-outcome-verifier30-v1/hybrid-outcome-verifier30-calibration-report.v1.md#decision), no new holdout is authorized, and F1a remains closed.
