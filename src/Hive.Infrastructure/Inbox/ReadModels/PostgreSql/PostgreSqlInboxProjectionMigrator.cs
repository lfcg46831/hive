...
public sealed partial class PostgreSqlInboxProjectionMigrator
{
    private const string ResourceMarker = ".Inbox.ReadModels.PostgreSql.Migrations.";
    private readonly NpgsqlDataSource _dataSource;
    private readonly Assembly _assembly;

    public PostgreSqlInboxProjectionMigrator(NpgsqlDataSource dataSource)
        : this(dataSource, typeof(PostgreSqlInboxProjectionMigrator).Assembly)
    {
    }

    internal PostgreSqlInboxProjectionMigrator(
    ...
    public async Task MigrateAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using (var bootstrap = new NpgsqlCommand(
            """
            SELECT pg_advisory_xact_lock(hashtext('hive.inbox.migrations'));
        ...
        await using (var command = new NpgsqlCommand(
            "SELECT version FROM inbox.schema_migrations;",
            connection,
            transaction))
        ...
        foreach (var migration in migrations.Where(item => !applied.Contains(item.Version)))
        {
            ...
            await using var record = new NpgsqlCommand(
                """
                INSERT INTO inbox.schema_migrations (version, name)
                VALUES (@version, @name);
                """,
                connection,
                transaction);
    ...
    private IReadOnlyList<MigrationResource> DiscoverMigrations()
    {
        ...
        if (resources.Length == 0)
        {
            throw new InvalidOperationException(
                "No embedded PostgreSQL inbox projection migrations were found.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"PostgreSQL inbox projection migration version {duplicate.Key} is duplicated.");
        }
    ...
    private static MigrationResource ParseResource(string resourceName)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Embedded PostgreSQL inbox projection migration '{resourceName}' must use NNN_name.sql naming.");
        }
        ...
        CancellationToken cancellationToken)
    {
        await using var stream = _assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                $"Embedded PostgreSQL inbox projection migration '{resourceName}' could not be opened.");
