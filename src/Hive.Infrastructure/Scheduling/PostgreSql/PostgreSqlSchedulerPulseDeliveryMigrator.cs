...
public sealed partial class PostgreSqlSchedulerPulseDeliveryMigrator
{
    private const string ResourceMarker = ".Scheduling.PostgreSql.Migrations.";
    private readonly NpgsqlDataSource _dataSource;
    private readonly Assembly _assembly;

    public PostgreSqlSchedulerPulseDeliveryMigrator(NpgsqlDataSource dataSource)
        : this(dataSource, typeof(PostgreSqlSchedulerPulseDeliveryMigrator).Assembly)
    {
    }

    internal PostgreSqlSchedulerPulseDeliveryMigrator(
    ...
    public async Task MigrateAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using (var bootstrap = new NpgsqlCommand(
            $"""
            SELECT pg_advisory_xact_lock(hashtext('hive.scheduler.migrations'));
            CREATE SCHEMA IF NOT EXISTS {SchedulerPulseDeliverySchema.SchemaName};
            CREATE TABLE IF NOT EXISTS {SchedulerPulseDeliverySchema.SchemaName}.schema_migrations (
        ...
        await using (var command = new NpgsqlCommand(
            $"SELECT version FROM {SchedulerPulseDeliverySchema.SchemaName}.schema_migrations;",
            connection,
            transaction))
        ...
        foreach (var migration in migrations.Where(item => !applied.Contains(item.Version)))
        {
            ...
            await using var record = new NpgsqlCommand(
                $"""
                INSERT INTO {SchedulerPulseDeliverySchema.SchemaName}.schema_migrations (version, name)
                VALUES (@version, @name);
                """,
                connection,
                transaction);
    ...
    private IReadOnlyList<MigrationResource> DiscoverMigrations()
    {
        ...
        {
            throw new InvalidOperationException("No embedded PostgreSQL scheduler migrations were found.");
        }
        ...
        if (duplicate is not null)
        {
            throw new InvalidOperationException(
                $"PostgreSQL scheduler migration version {duplicate.Key} is duplicated.");
        }
    ...
    private static MigrationResource ParseResource(string resourceName)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Embedded PostgreSQL scheduler migration '{resourceName}' must use NNN_name.sql naming.");
        }
        ...
        CancellationToken cancellationToken)
    {
        await using var stream = _assembly.GetManifestResourceStream(resourceName)
            ?? throw new InvalidOperationException(
                $"Embedded PostgreSQL scheduler migration '{resourceName}' could not be opened.");
