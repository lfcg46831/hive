...
public sealed record DirectiveEvaluationInstruction
{
    public DirectiveEvaluationInstruction(
        int rubricVersion,
        string content,
        IEnumerable<DirectiveEvaluationDimension>? envelopeDimensions = null)
    {
        if (rubricVersion <= 0)
        {
            throw new ArgumentOutOfRangeException(
        ...
        Content = string.IsNullOrWhiteSpace(content)
            ? throw new ArgumentException(
                ...
                nameof(content))
            : content.Trim();
        EnvelopeDimensions = envelopeDimensions is null
            ? []
            : envelopeDimensions.ToImmutableArray();
        if (EnvelopeDimensions.Any(dimension => dimension is null) ||
            EnvelopeDimensions.Select(dimension => dimension.Id)
                .Distinct(StringComparer.Ordinal)
                .Count() != EnvelopeDimensions.Length)
        {
            throw new ArgumentException(
                "Evaluation dimensions must be non-null and have unique ids.",
                nameof(envelopeDimensions));
        }

        EnvelopeDimensions = EnvelopeDimensions
    ...
    public ImmutableArray<DirectiveEvaluationDimension> EnvelopeDimensions { get; }
}

public sealed record DirectiveEvaluationDimension
{
    public DirectiveEvaluationDimension(
        string id,
        bool singleLabel,
        IEnumerable<string> labels)
    {
        Id = string.IsNullOrWhiteSpace(id)
            ? throw new ArgumentException(
                "Evaluation dimension id is required.",
                nameof(id))
            : id;
        ...
        {
            throw new ArgumentException(
                "Evaluation labels must be non-empty, non-blank and unique.",
                nameof(labels));
        }
    }
...
public interface IDirectiveEvaluationInstructionPort
{
    DirectiveEvaluationInstruction? Resolve(
