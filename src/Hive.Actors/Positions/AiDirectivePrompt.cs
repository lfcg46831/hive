...
internal static class AiDirectivePrompt
{
        ...
        EffectiveDirectiveExecutionPolicy executionPolicy)
    {
        ...
        if (evidenceContext is not null)
        {
            ...
            lines.Add(
                $"Grounding evidence_references may use only source \"{OutcomeEvidenceSourceContract.ToWireValue(OutcomeEvidenceSource.DirectiveInput)}\" and these exact bounded references: {EvidenceReferenceVocabulary(evidenceContext)}.");
            lines.Add(
