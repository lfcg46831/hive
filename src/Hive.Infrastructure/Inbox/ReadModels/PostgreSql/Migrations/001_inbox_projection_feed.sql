CREATE TABLE inbox.projection_checkpoints (
...
INSERT INTO inbox.projection_checkpoints (
    subscription,
    source_offset,
    updated_at_utc)
VALUES
...
CREATE TABLE inbox.projection_facts (
...
CREATE INDEX projection_facts_sequence_idx
    ON inbox.projection_facts (sequence_id);

CREATE INDEX projection_facts_position_idx
    ON inbox.projection_facts (
        organization_id,
        position_id,
        sequence_id);

CREATE INDEX projection_facts_message_idx
    ON inbox.projection_facts (
        organization_id,
        message_id,
        sequence_id)
