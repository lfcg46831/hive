...
internal sealed partial class PostgreSqlGitHubIssuesInboundMigrator
{
    private const string ResourceMarker = ".PostgreSql.Migrations.";
    private readonly NpgsqlDataSource _dataSource;
    private readonly Assembly _assembly;

    public PostgreSqlGitHubIssuesInboundMigrator(NpgsqlDataSource dataSource)
        : this(dataSource, typeof(PostgreSqlGitHubIssuesInboundMigrator).Assembly)
    {
    }

    internal PostgreSqlGitHubIssuesInboundMigrator(
    ...
    public async Task MigrateAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using (var bootstrap = new NpgsqlCommand(
            $"""
            SELECT pg_advisory_xact_lock(hashtext('hive.github-connector.migrations'));
            CREATE SCHEMA IF NOT EXISTS {GitHubIssuesInboundSchema.SchemaName};
            CREATE TABLE IF NOT EXISTS {GitHubIssuesInboundSchema.SchemaName}.schema_migrations (
        ...
        await using (var command = new NpgsqlCommand(
            $"SELECT version FROM {GitHubIssuesInboundSchema.SchemaName}.schema_migrations;",
            connection,
            transaction))
        ...
        foreach (var migration in migrations.Where(item => !applied.Contains(item.Version)))
        {
            ...
            await using var record = new NpgsqlCommand(
                $"""
                INSERT INTO {GitHubIssuesInboundSchema.SchemaName}.schema_migrations (version, name)
                VALUES (@version, @name);
                """,
                connection,
                transaction);
    ...
    private IReadOnlyList<MigrationResource> DiscoverMigrations()
    {
        ...
        if (migrations.Length == 0)
        {
            throw new InvalidOperationException(
                "No embedded PostgreSQL GitHub connector migrations were found.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"PostgreSQL GitHub connector migration version {duplicate.Key} is duplicated.");
        }
    ...
    private static MigrationResource ParseResource(string resourceName)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Embedded PostgreSQL GitHub connector migration '{resourceName}' must use NNN_name.sql naming.");
        }
        ...
        CancellationToken cancellationToken)
    {
        await using var stream = _assembly.GetManifestResourceStream(resourceName) ??
            throw new InvalidOperationException(
                $"Embedded PostgreSQL GitHub connector migration '{resourceName}' could not be opened.");
